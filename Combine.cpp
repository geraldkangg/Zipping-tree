...
Vector<DataPoint> merge(const Vector<DataPoint>& seq1, const Vector<DataPoint>& seq2) {
    ...
    int index1 = 0;
    int index2 = 0;
    Vector<DataPoint> finalMerge;

    while (index1 < seq1.size() && index2 < seq2.size()) {
        if (seq1[index1].weight > seq2[index2].weight) {
            finalMerge += seq1[index1];
            index1 += 1;
        }
        else {
            finalMerge += seq2[index2];
            index2 += 1;
        }
    }
...
Vector<DataPoint> combine(const Vector<Vector<DataPoint>>& sequences) {
    ...
    Vector<Vector<DataPoint>> split1 = sequences.subList(0, sequences.size()/2);

    Vector<Vector<DataPoint>> split2 = sequences.subList(sequences.size()/2);

    Vector<DataPoint> seq1 = combine(split1);

    Vector<DataPoint> seq2 = combine(split2);
